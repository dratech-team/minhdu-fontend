...
@Component({
  templateUrl: 'poultry-food-dialog.component.html'
})
export class PoultryFoodDialogComponent implements OnInit {
  formGroup!: FormGroup;
  ...
  onSubmit() {
    ...
    if (this.data) {
      this.store.dispatch(PoultryFoodAction.updatePoultryFood({ poultryFood: poultryFood, id: this.data.id }));
    } else {
      this.store.dispatch(PoultryFoodAction.addPoultryFood({ poultryFood: poultryFood }));
    }
