  ...
  ngOnInit() {
    if (this.data?.consignment) {
      this.formGroup = this.formBuilder.group({
        ...
        exp: [
          this.datePipe.transform(this.data.consignment.exp, 'yyyy-MM-dd'),
          Validators.required,
        ],
        mfg: [
          this.datePipe.transform(this.data.consignment.mfg, 'yyyy-MM-dd'),
          Validators.required,
        ],
  ...
  onSubmit() {
    ...
    if (this.data?.isUpdate && this.data.consignment) {
      this.action$.dispatch(
        ConsignmentActions.update({
          id: this.data.consignment.id,
          updates: consignment,
        })
      );
    } else {
