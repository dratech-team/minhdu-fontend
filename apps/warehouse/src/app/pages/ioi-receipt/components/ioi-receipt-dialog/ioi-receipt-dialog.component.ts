      ...
      this.formGroup = this.formBuilder.group({
        accountedAt: [
          this.datePipe.transform(
            this.data.ioiReceipt.accountedAt,
            'yyyy-MM-dd'
          ),
          Validators.required,
        ],
        billedAt: [
          this.datePipe.transform(this.data.ioiReceipt.billedAt, 'yyyy-MM-dd'),
          Validators.required,
        ],
        billCode: [this.data.ioiReceipt.billCode, Validators.required],
        discount: [
          this.data.ioiReceipt?.discount && this.data.ioiReceipt?.discountType
            ? this.data.ioiReceipt.discountType === DiscountTypeEnum.PERCENT
              ? this.data.ioiReceipt.discount * 100
              : this.data.ioiReceipt.discountType === DiscountTypeEnum.CASH
        ...
        orderedAt: [
          this.datePipe.transform(this.data.ioiReceipt.orderedAt, 'yyyy-MM-dd'),
          Validators.required,
        ],
        importedAt: [
          this.datePipe.transform(
            this.data.ioiReceipt.importedAt,
            'yyyy-MM-dd'
          ),
          Validators.required,
        ],
        completedAt: [
          this.datePipe.transform(
            this.data.ioiReceipt.completedAt,
            'yyyy-MM-dd'
          ),
          Validators.required,
        ],
        approvedAt: [
          this.datePipe.transform(
            this.data.ioiReceipt.approvedAt,
            'yyyy-MM-dd'
          ),
          Validators.required,
        ],
  ...
  onSubmit() {
    ...
    if (this.data?.isUpdate && this.data.ioiReceipt) {
      this.actions$.dispatch(
        IoiReceiptActions.update({
          id: this.data.ioiReceipt.id,
          updates: stockUpdate,
        })
      );
    } else {
