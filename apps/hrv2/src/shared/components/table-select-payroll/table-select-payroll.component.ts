...
export class TableSelectPayrollComponent implements OnInit {
  ...
  ngOnInit() {
    this.formGroup.value.payrollIds.forEach((id: number) =>
  ...
  refreshCheckedStatus(): void {
    this.checked = this.payrolls.every(({ id }) =>
      this.payrollIdsSelected.has(id)
    );
    this.indeterminate =
      this.payrolls.some(({ id }) => this.payrollIdsSelected.has(id)) &&
      !this.checked;
  }
  ...
  onItemChecked(id: number, checked: boolean): void {
    ...
    this.formGroup
      .get('payrollIds')
      ?.setValue(Array.from(this.payrollIdsSelected));
  }
