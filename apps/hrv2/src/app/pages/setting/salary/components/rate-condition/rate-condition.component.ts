...
export class RateConditionComponent implements OnInit {
  ...
  onLoad(loadMore: boolean): any {
    ...
    this.service.pagination({
      ...
      this.rateConditions = loadMore ? this.rateConditions.concat(val.data) : val.data
    })
  }
  ...
  onAdd() {
    this.modal.create({
      ...
      if (rateConditionEntity) {
        this.total += 1
        this.rateConditions = this.rateConditions.concat([rateConditionEntity])
    ...
    this.modal.warning({
      ...
      nzOnOk: () => {
        this.service.delete(rateCondition.id).subscribe(_ => {
          this.total -= 1
          const index = this.rateConditions.findIndex(e => e.id === rateCondition.id)
