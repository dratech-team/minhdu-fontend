...
export class ModalRateConditionComponent implements OnInit {
  ...
  onSubmit(): any {
    ...
    (this.data?.update
      ? this.service.update({
          id: this.data.update.rateCondition.id,
          updates: _.omit(this.formGroup.value, ['inputWith']),
        })
      : this.service.addOne({
          body: _.omit(this.formGroup.value, ['inputWith']),
        })
    )
      .pipe(catchError((err) => this.onSubmitErr(err)))
      .subscribe((val) => {
  ...
  private onSubmitSuccess(rateCondition: RateConditionEntity) {
    ...
    this.message.success(
      this.data?.update ? 'Cập nhật thành công' : 'Tạo thành công'
    );
    this.modalRef.close(
      this.data?.update
        ? Object.assign(
            this.data.update.rateCondition,
            _.omit(this.formGroup.value, ['inputWith'])
          )
        : rateCondition
    );
  }
