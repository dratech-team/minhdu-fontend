  ...
  transform(settingSalary: SalarySettingEntity): any {
    switch (settingSalary.type) {
      ...
      case SalaryTypeEnum.HOLIDAY:
        return (
          (settingSalary.prices && settingSalary.prices.length > 0
            ? this.pricesPipe.transform(settingSalary.prices)
            : this.salaryTypePipe.transform(
                settingSalary.totalOf,
                'transform'
              )) +
          (settingSalary.workday
            ...
            : settingSalary.type === SalaryTypeEnum.OVERTIME &&
              settingSalary.totalOf.length === 0
            ? ''
            : ' / Ngày công chuẩn')
        );
      default:
