...
import {recipesConstant, recipesConstantOvertime, salariesConstant} from "../constants";
...
export class SalaryTypePipe implements PipeTransform {
  transform(salariesEnum:SalaryTypeEnum[] , type: 'filter'|'transform', blockType?: SalaryTypeEnum): any {
    if(type === 'transform'){
      ...
      if (salariesEnum.length > 0) {
        return( blockType === SalaryTypeEnum.OVERTIME
