...
export class SettingRankComponent implements OnInit {
  ...
  added$ = this.settingRankQuery.select(state => state.added).pipe(tap(val => {
    if (val) {
  ...
  ngOnInit() {
    this.onload(false)
    this.formGroup.get('rank')?.valueChanges.subscribe(val =>
      this.formGroup.get('rank')?.setValue(val.toUpperCase(), {emitEvent: false})
    )
  }
  ...
  onload(isPaginate: boolean) {
    ...
    this.actions$.dispatch(SettingRankActions.loadAll({
