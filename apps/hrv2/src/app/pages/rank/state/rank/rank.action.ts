 ...
 * */
const addOne = createAction('[RANK] Add One', props<AddRankDto>());

const loadAll = createAction('[RANK] Load All', props<SearchRankDto>());

const loadOne = createAction('[RANK] Load One', props<LoadOneRankDto>());

/**
 * @deprecated
 * thứ hạng xếp loại người dung không được cập nhật , để uopdate  phòng trường hợp thay đổi logic
 * */
const update = createAction('[RANK] Update', props<UpdateRankDto>());

/**
 * @deprecated
 * thứ hạng xếp loại người dung không được xoá , để remove  phòng trường hợp thay đổi logic
 * */
const remove = createAction('[RANK] Remove', props<RemoveRankDto>());

const error = createAction('[RANK] Error', props<{ error: string }>());
