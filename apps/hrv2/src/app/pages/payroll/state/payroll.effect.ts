...
export class PayrollEffect {
        ...
        .pipe(
          map((res) => {
            return Object.assign(res, {
              data: res.data.map((payroll) =>
                Object.assign({}, this.mapToPayroll(payroll), {
                  expand: this.payrollStore.getValue().expandAll,
                  basics: payroll.salariesv2
                    ? payroll.salariesv2.filter(
                        (item) => item.type !== SalaryTypeEnum.STAY
                      )
                    : [],
        ...
        map((res) => {
          if (this.payrollQuery.getEntity(props.id)?.idUpdate?.overtime) {
            this.payrollStore.update(props.id, (entity) => {
              return entity.idUpdate?.overtime
                ? {
                    overtimes: arrayUpdate(
                      entity.overtimes,
                      entity.idUpdate.overtime,
                      res.overtimes.find(
                        (salary) => salary.id === entity.idUpdate?.overtime
                      ) || ({} as SalaryEntity)
                    ),
  ...
  private mapToPayroll(payroll: PayrollEntity): PayrollEntity {
    const basics = payroll.salariesv2.filter(
      (item) =>
        item.type === SalaryTypeEnum.BASIC ||
        item.type === SalaryTypeEnum.BASIC_INSURANCE
    );
    const stays = payroll.salariesv2.filter(
      (item) => item.type === SalaryTypeEnum.STAY
    );
    ...
    return Object.assign(payroll, {
      ...
      overtimes: this.sortDatetime(payroll.overtimes)?.map((overtime) =>
        Object.assign(overtime, { expand: false })
      ),
  ...
  private getTotalOvertimeOrAbsent(
    salary: (AbsentSalaryEntity | OvertimeSalaryEntity)[]
  ): TotalSalary {
    return salary.reduce(
      (a, b) => {
        ...
        const partial =
          ...
          unit === DatetimeUnitEnum.TIMES
            ? b.partial === PartialDayEnum.ALL_DAY ||
              b.partial === PartialDayEnum.NIGHT
              ? b.duration
              : b.duration / 2
            : 0;
        ...
        const hour =
          a.duration.hour +
          this.convertMinutePipe.transform(a.duration.minute).hour +
          (unit === DatetimeUnitEnum.HOUR
            ? this.convertMinutePipe.transform(b.duration * 60).hour
            : 0);
        const minute =
          this.convertMinutePipe.transform(a.duration.minute).minute +
    ...
    return salary?.reduce(
      (a, b) => {
        return {
          duration:
            a.duration +
            (b.partial && b.partial === PartialDayEnum.ALL_DAY
              ? b.duration
              : b.duration / 2),
        };
      },
    ...
    return salary?.reduce(
      (a, b) => {
        return {
          duration:
            a.duration +
            (b.setting.startedAt && b.setting.endedAt
