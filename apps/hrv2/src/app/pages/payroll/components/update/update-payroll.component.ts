...
export class UpdatePayrollComponent implements OnInit {
  ...
  branches$ = this.branchQuery.selectAll().pipe(
    map((branches) => {
      if (branches.length === 1) {
        ...
        this.positions = branches.find(
          (branch) => branch.name === this.data.payroll.branch
        )?.positions;
      }
      return branches;
    })
  ...
  compareFN = (o1: any, o2: any) =>
    typeof o1 === 'string' && o2 ? o1 === o2.name : o1.id === o2.id;
  compareRecipe = (o1: any, o2: any) =>
    o1 && o2 ? o1 === o2.value || o1.value === o2.value : o1 === o2;
  ...
  mapPayroll() {
    ...
    return Object.assign(
      payroll,
      value.tax ? { tax: value.tax / 100 } : {},
      value.position?.id ? { positionId: value.position.id } : {},
      value.branch?.id ? { branchId: value.branch.id } : {},
      value.recipeType?.value ? { recipeType: value.recipeType.value } : {}
    );
  }
}
