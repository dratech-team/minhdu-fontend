    ...
    switchMap((props: SearchContractDto) => {
      ...
      Object.assign(props.search,
        {
          ...
          skip: props.isPaginate ? this.contractQuery.getCount() : PaginationDto.skip
        }
      )
      return this.rankService.pagination(props).pipe(
        map((res) => {
          ...
          this.contractStore.update(state => (Object.assign({
                ...state, total: res.total, remain: res.total - this.contractQuery.getCount()
              }, props.isPaginate
                ? {loadMore: false}
                : {loading: false}
            )
