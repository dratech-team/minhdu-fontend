...
export class TableSalarySelectedComponent implements OnInit {
  ...
  updateCheckedSet(id: number, checked: boolean): void {
    ...
    this.formGroup
      .get('salaryIds')
      ?.setValue(Array.from(this.salaryIdsSelected));
  }

  refreshCheckedStatus(): void {
    this.checked = this.salaries.every(({ id }) =>
      this.salaryIdsSelected.has(id)
    );
    this.indeterminate =
      this.salaries.some(({ id }) => this.salaryIdsSelected.has(id)) &&
      !this.checked;
  }
  ...
  onAllChecked(checked: boolean): void {
    this.salaries.forEach(({ id }) => this.updateCheckedSet(id, checked));
    this.refreshCheckedStatus();
    this.formGroup
      .get('salaryIds')
      ?.setValue(Array.from(this.salaryIdsSelected));
