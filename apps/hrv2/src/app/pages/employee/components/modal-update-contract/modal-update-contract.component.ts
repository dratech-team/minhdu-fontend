...
export class ModalUpdateContractComponent implements OnInit {
  ...
  ngOnInit() {
    this.formGroup = this.formBuilder.group({
      createdAt: [
        this.data.contracts.slice(-1)[0]?.createdAt,
        Validators.required,
      ],
      expiredAt: [
        this.data.contracts.slice(-1)[0]?.expiredAt,
        Validators.required,
      ],
    });
  }
  ...
  onSubmit() {
    ...
    if (this.data.contracts?.length > 0) {
      this.contractService
        .update(this.data.contracts.slice(-1)[0].id, contract)
        .pipe(
          catchError((err) => {
      ...
      nzOnOk: () => {
        this.data.contracts.map((contract: ContractEntity) => {
          this.actions$.dispatch(
            EmployeeActions.removeContracts({
              id: contract.id,
              employeeId: this.data.employeeId,
            })
          );
  ...
  private onSubmitSuccess() {
    this.submitting = true;
    this.actions$.dispatch(
      EmployeeActions.loadOne({ id: this.data.employeeId })
    );
    this.modalRef.close();
  }
}
