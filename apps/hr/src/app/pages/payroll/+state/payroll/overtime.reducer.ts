...
export const adapterOvertime: EntityAdapter<PayrollSalary> =
  createEntityAdapter<PayrollSalary>();
export const initialOvertime = adapterOvertime.getInitialState({
  loaded: false,
  added: false,
  adding: false,
});
export const OvertimeReducer = createReducer(
  ...
  on(PayrollAction.filterOvertimeSuccess, (state, action) =>
    adapterOvertime.setOne(action.overtime, {
      ...state,
      loaded: true,
      added: true,
      adding: false,
