...
export class DetailPayrollComponent implements OnInit {
  type = SalaryTypeEnum;
  payroll$ = this.store
    .pipe(select(selectCurrentPayroll(this.getPayrollId)))
    .pipe(
      map((payroll) => {
        if (payroll) {
          ...
          this.sortedSalaryOver = JSON.parse(
            JSON.stringify(
              payroll?.salaries.filter(
                (salary) => salary.type === SalaryTypeEnum.OVERTIME
              )
            )
          );
        }
    ...
    switch (type) {
      case SalaryTypeEnum.BASIC: {
        ...
        this.dialog.open(
          ...
          Object.assign(config, { width: 'fit-content' })
        );
        break;
      }
      ...
      case SalaryTypeEnum.ABSENT: {
        this.dialog.open(
          ...
          Object.assign(config, { width: '600px' })
        );
        break;
      }
  ...
  removeSalary(id: number, payrollId: number) {
    this.dialog
      ...
      .subscribe((value) => {
        if (value) {
          this.store.dispatch(
            PayrollAction.deleteSalary({ id: id, PayrollId: payrollId })
          );
        }
      });
  }
  ...
  prePayroll(payroll: Payroll) {
    ...
    if (indexPayrollCurrent > 0) {
      ...
      this.router
        .navigate([
          'phieu-luong/chi-tiet-phieu-luong',
          payrollIds[payrollIds.length - 1],
        ])
        .then();
    }
  }
    ...
    const ref = this.dialog.open(DialogSharedComponent, {
      ...
      data: {
        ...
        description: `Bạn muốn ${
          payroll.taxed ? 'tắt' : 'bật'
        } trừ thuế cho phiếu lương của tháng
