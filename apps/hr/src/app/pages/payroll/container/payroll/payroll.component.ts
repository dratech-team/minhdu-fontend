  ...
  formGroup = new FormGroup({
    ...
    createdAt: new FormControl(
      this.datePipe.transform(
        getState<Date>(selectedCreateAtPayroll, this.store),
        'yyyy-MM'
      )
    ),
    position: new FormControl(
      getState<string>(selectedPositionPayroll, this.store)
    ),
    branch: new FormControl(
      getState<string>(selectedBranchPayroll, this.store)
    ),
  });
  selectPayroll = new FormControl(
    getState<PayrollEnum>(selectedTypePayroll, this.store)
  );
  selectedPayroll: PayrollEnum = getState<PayrollEnum>(
    selectedTypePayroll,
    this.store
  );
  branchName = getState<string>(selectedBranchPayroll, this.store);
  positionName = getState<string>(selectedPositionPayroll, this.store);
      ...
      .pipe(
        map((val) => {
          if (
            ...
            this.formGroup
              .get('createdAt')!
              .patchValue(this.datePipe.transform(new Date(), 'yyyy-MM'));
            takeUntil(this.stop$);
          } else {
            return val;
          }
