...
export class DialogOvertimeComponent implements OnInit {
  ...
  onSubmit(): any {
    ...
    const salary = {
      ...
      rate: value.rate,
      times:
        value.unit === this.datetimeUnitEnum.DAY && value.days > 1
          ? value.days
          : value.times,
    ...
    if (this.data?.isUpdate) {
      ...
      Object.assign(salary, {
        allowanceDeleted: !!(
          !this.isAllowanceOvertime && this.data.salary?.allowance
        ),
      });
      if (this.data?.updateMultiple) {
        Object.assign(salary, {
      ...
      if (value.unit === DatetimeUnitEnum.HOUR && !value.times) {
        return this.snackBar.open('chưa nhập số giờ tăng ca', '', {
          duration: 2000,
        });
      }
      this.store.dispatch(
    ...
    this.store.dispatch(
      TemplateOvertimeAction.loadALlTemplate(
        this.formGroup.value.unit &&
          this.formGroup.value.unit !== DatetimeUnitEnum.OPTION
          ? {
