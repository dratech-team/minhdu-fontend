...
export class PickPayrollOvertimeComponent implements OnInit, OnChanges {
    ...
    if (changes.allowPayrollsSelected) {
      if (changes.allowPayrollsSelected.currentValue.length === 0) {
        ...
        this.isSelectAllowance =
          this.payrolls !== null &&
          this.payrolls.every((e) =>
            this.allowPayrollsSelected.some((item) => item.id === e.id)
          );
      }
    }
        ...
        tap((val) => {
          ...
          this.payrollService
            ...
            .subscribe((res) => {
              this.loading = false;
              this.totalPayroll = res.total;
              this.isSelectAllPayroll = checkIsSelectAllInit(
                res.data,
                this.payrollsSelected
              );
              ...
              if (res.data.length > 0) {
                res.data.forEach((payroll: Payroll) => {
                  if (this.isSelectAllPayroll) {
                    if (
                      !this.payrollsSelected.some((e) => e.id === payroll.id)
                    ) {
                      this.payrollsSelected.push(payroll);
                    }
                    if (this.isSelectAllowance) {
                      if (
                        !this.allowPayrollsSelected.some(
                          (e) => e.id === payroll.id
                        )
                      ) {
  ...
  someCompleteEmployee(): boolean {
    return (
      this.payrolls.filter((e) =>
        this.payrollsSelected.some((item) => item.id === e.id)
      ).length > 0 && !this.isSelectAllPayroll
    );
  }
  ...
  setAllAllowance(select: boolean) {
    ...
    if (select) {
      ...
      this.payrolls.forEach((payroll) => {
        const indexAllowance = this.allowPayrollsSelected.findIndex(
          (emp) => emp.id === payroll.id
        );
        if (indexAllowance > -1) {
        ...
        if (respone.data.length > 0) {
          respone.data.map((payroll) => {
            if (this.isSelectAllPayroll) {
              ...
              if (this.isSelectAllowance) {
                if (
                  this.allowPayrollsSelected.every(
                    (val) => val.id !== payroll.id
                  )
                ) {
                  this.allowPayrollsSelected.push(payroll);
                }
