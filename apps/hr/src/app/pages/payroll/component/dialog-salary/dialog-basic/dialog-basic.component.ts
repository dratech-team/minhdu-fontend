...
export class DialogBasicComponent implements OnInit {
  ...
  salaries = [
    ...
    {
      title: 'Lương TN quản lý thêm',
      type: SalaryTypeEnum.BASIC_TRUST_MANAGER,
    },
  ];
  ...
  ngOnInit(): void {
    ...
    this.store.dispatch(
      TemplateSalaryAction.loadALlTemplate({ salaryType: SalaryTypeEnum.BASIC })
    );
    if (this.data?.salary) {
      this.formGroup = this.formBuilder.group(
        this.data.isHistorySalary
          ? {
  ...
  onSubmit(): any {
    ...
    if (this.data?.isUpdate) {
          ...
          .subscribe((val) => {
            if (val) {
              ...
              this.dialogRef.close({
                title: this.salariesSelected[0].salary.title,
              });
            }
          });
      } else {
      ...
      if (this.data.isHistorySalary) {
        ...
        this.store.dispatch(
          EmployeeAction.updateHistorySalary({
            id: this.data.salary.id,
            salary: salary,
