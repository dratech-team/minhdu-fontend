...
export const adapter: EntityAdapter<SalarySetting> =
  createEntityAdapter<SalarySetting>();

export const initialTemplateBasic = adapter.getInitialState({
  loaded: false,
  added: false,
  adding: false,
  total: 0,
...
export const templateSalaryReducer = createReducer(
  ...
  on(TemplateSalaryAction.loadMoreTempLateSuccess, (state, action) =>
    adapter.addMany(action.templateSalary, {
      ...state,
      loaded: true,
      total: action.total,
    })
  ...
  on(TemplateSalaryAction.AddTemplateSuccess, (state, action) =>
    adapter.setOne(action.template, {
      ...state,
      loaded: true,
      added: true,
      adding: false,
