...
export class TemplateSalaryEffect {
  ...
  loadInit$ = createEffect(() =>
    this.action$.pipe(
      ...
      switchMap((props: any) =>
        this.templateSalaryService.pagination(
          Object.assign(JSON.parse(JSON.stringify(props.templateSalaryDTO)), {
            type: props.templateSalaryDTO.salaryType
              ? props.templateSalaryDTO.salaryType
              : '',
          })
        )
      ),
  ...
  addTemplate$ = createEffect(() =>
    this.action$.pipe(
      ...
      switchMap((pram) =>
        this.templateSalaryService.addOne(pram.template).pipe(
          map((_) =>
            TemplateSalaryAction.loadInit({
              templateSalaryDTO: { take: 30, skip: 0 },
            })
          ),
          catchError((err) => {
  ...
  updateTemplate = createEffect(() =>
    this.action$.pipe(
      ...
      switchMap((pram) =>
        this.templateSalaryService.update(pram.id, pram.template).pipe(
          map((_) =>
            TemplateSalaryAction.loadInit({
              templateSalaryDTO: { take: 30, skip: 0 },
            })
  ...
  deleteTemplate$ = createEffect(() =>
    this.action$.pipe(
      ...
      switchMap((pram) => {
        return this.templateSalaryService
          .delete(pram.id)
          .pipe(
            map((_) =>
              TemplateSalaryAction.loadInit({
