...
export const adapter: EntityAdapter<TemplateOvertime> =
  createEntityAdapter<TemplateOvertime>();
...
export const templateOvertimeReducer = createReducer(
  ...
  on(TemplateOvertimeAction.loadMoreTempLateSuccess, (state, action) =>
    adapter.addMany(action.templateOvertimes, {
      ...state,
      loaded: true,
      total: action.total,
    })
  ...
  on(TemplateOvertimeAction.AddTemplateSuccess, (state, action) =>
    adapter.setOne(action.templateOvertime, {
      ...state,
      added: true,
      adding: false,
      total: state.total + 1,
