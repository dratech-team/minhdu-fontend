...
export class TemplateOvertimeEffect {
  ...
  loadInit$ = createEffect(() =>
    this.action$.pipe(
      ...
      switchMap((props) => {
        return this.templateOvertimeService.pagination(
          props.templateOvertimeDTO
        );
      }),
      map((responsePagination) => {
  ...
  loadMore$ = createEffect(() =>
    this.action$.pipe(
      ...
      map(([props, skip]) =>
        Object.assign(JSON.parse(JSON.stringify(props.templateOvertimeDTO)), {
          skip: skip,
        })
      ),
      switchMap((props) => {
  ...
  addTemplate$ = createEffect(() =>
    this.action$.pipe(
      ...
      switchMap((pram) =>
        this.templateOvertimeService.addOne(pram.template).pipe(
          map((_) =>
  ...
  updateTemplate = createEffect(() =>
    this.action$.pipe(
      ...
      switchMap((pram) =>
        this.templateOvertimeService
          .update(pram.id, pram.templateOvertime)
          .pipe(
            map((_) =>
              TemplateOvertimeAction.loadInit({
                templateOvertimeDTO: { take: 30, skip: 0 },
  ...
  deleteTemplate$ = createEffect(() =>
    this.action$.pipe(
      ...
      switchMap((pram) => {
        return this.templateOvertimeService
          .delete(pram.id)
          .pipe(
            map((_) =>
              TemplateOvertimeAction.loadInit({
