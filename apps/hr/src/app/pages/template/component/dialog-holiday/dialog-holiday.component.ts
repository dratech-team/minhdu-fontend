  ...
  ngOnInit() {
    ...
    if (this.data?.holiday) {
      ...
      this.formGroup = this.formBuilder.group({
        ...
        rate: [
          this.data?.isUpdate ? this.data.holiday.rate : 1,
          Validators.required,
        ],
        isConstraint: [
          this.data?.isUpdate ? this.data?.holiday?.isConstraint : true,
        ],
    ...
    this.formGroup
      .get('rate')
      ?.valueChanges.subscribe((rate) => (this.hidePrice = rate <= 1));
  }
    ...
    if (this.data?.isUpdate) {
      this.store.dispatch(
        HolidayAction.UpdateHoliday({
          ...
          holiday: holiday,
          updateDetail: this.data.updateDetail,
        })
      );
    } else {
