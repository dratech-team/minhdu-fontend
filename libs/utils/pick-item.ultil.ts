...
export const someComplete = (items: any [], itemsSelected: any [], isSelectAll: boolean): boolean => {
  if (items.length === 0) {
    ...
    return items.filter(val => itemsSelected.some(x => x.id === val.id)).length > 0 && !isSelectAll;
  }
};

export const checkIsSelectAllInit = (itemSub: any [], itemsSelected: any []) => {
  return itemSub.every(item =>
      itemsSelected.some(x => x.id === item.id))
    && itemSub.length > 0 && itemsSelected.length > 0;
};

export const handleValSubPickItems = (valSub: any[], items: any[], itemSelected: any [], isSelectAll: boolean) => {
  valSub.forEach((val) => {
    if (isSelectAll) {
