...
export const filterSalaryPayroll = (salaryPayrolls: SalaryPayroll[], salary: Salary) => {
  return salaryPayrolls.filter(salaryPayroll =>
    (salaryPayroll.salary.title === salaryPayroll.salary.title
      && (
        salary.type === SalaryTypeEnum.BASIC ||salaryPayroll.salary.type === SalaryTypeEnum.STAY ?
          moment( salaryPayroll.salary.datetime).isSame(salaryPayroll.salary.datetime,'month') :
          moment( salaryPayroll.salary.datetime).isSame(salaryPayroll.salary.datetime,'day')
      )
