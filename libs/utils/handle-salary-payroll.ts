...
export const filterSalaryPayroll = (
  ...
  return salaryPayrolls.filter((salaryPayroll) => {
    return (
      ...
      isSame(
        salaryPayroll.salary.datetime,
        salary.datetime,
        salary.type === SalaryTypeEnum.BASIC ||
          salaryPayroll.salary.type === SalaryTypeEnum.STAY
          ? 'month'
          : 'day'
      ) &&
...
const isSame = (date1: Date, date2: Date, type: 'day' | 'month') =>
  moment(date1).isSame(date2, type);

export const updateSelectOneSalaryPayroll = (
  event: boolean,
  ...
  if (event) {
    ...
    const index = salaryPayrolls.findIndex(
      (value) => value.salary.id === salaryPayroll.salary.id
    );
    salaryPayrolls.splice(index, 1);
  }
};

export const filterSameSalary = (
  salaries: SalaryEntity[],
  salary: SalaryEntity
) => {
  return salaries.filter((item) => {
    return (
      salary?.setting?.id === item?.setting?.id &&
