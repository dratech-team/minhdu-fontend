...
export class DepartmentEffects {
  ...
  loadDepartments$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map((departments) =>
        DepartmentAction.loadDepartmentSuccess({ departments })
      ),
  ...
  addDepartment$ = createEffect(
    () =>
      this.actions$.pipe(
        ...
        switchMap((param) =>
          this.departmentService.addOne(param.department).pipe(
  ...
  updateDepartment$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((param) =>
        this.departmentService.update(param.id, { name: param.name }).pipe(
          map((_) => OrgchartActions.init()),
          catchError((err) => throwError(err))
        )
      )
  ...
  deleteDepartment$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((param) =>
        this.departmentService.delete(param.id).pipe(
          map((_) => OrgchartActions.init()),
          catchError((err) => throwError(err))
        )
      )
