...
export const positionAdapter: EntityAdapter<Position> =
  createEntityAdapter<Position>();
...
const positionReducer = createReducer(
  ...
  on(PositionActions.updatePositionSuccess, (state, { position }) =>
    positionAdapter.updateOne(
      { id: position.id, changes: position },
      { ...state, added: true, error: null }
    )
  ),
  on(PositionActions.loadPositionSuccess, (state, { position }) => {
    return positionAdapter.setAll(position, {
      ...state,
      loaded: true,
      added: true,
    });
